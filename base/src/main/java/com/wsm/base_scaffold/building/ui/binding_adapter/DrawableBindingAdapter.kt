...
object DrawableBindingAdapter {
    ...
    fun create(
        ...
        if (startColor != null && endColor != null) {
            ...
            if (gradientType == GradientType.RADIAL) {
                drawable.setGradientCenter(radialCenterX ?: .5f,
                        radialCenterY ?: .5f)
        ...
        if (strokeWidth > 0) {
            drawable.setStroke(dip2px(strokeWidth), strokeColor, dip2px(strokeDash).toFloat(), dip2px(strokeDashGap).toFloat())
        }
        if (radius <= 0) {
            val radiusEach = floatArrayOf(dip2px(radiusLT).toFloat(), dip2px(radiusLT).toFloat(), dip2px(radiusRT).toFloat(), dip2px(radiusRT).toFloat(),
                    dip2px(radiusRB).toFloat(), dip2px(radiusRB).toFloat(), dip2px(radiusLB).toFloat(), dip2px(radiusLB).toFloat())
    ...
    private fun setRingValue(drawable: GradientDrawable,
    ...
    @IntDef(Orientation.TOP_BOTTOM, Orientation.TR_BL, Orientation.RIGHT_LEFT, Orientation.BR_TL, Orientation.BOTTOM_TOP, Orientation.BL_TR, Orientation.LEFT_RIGHT, Orientation.TL_BR)
    @Retention(AnnotationRetention.SOURCE)
